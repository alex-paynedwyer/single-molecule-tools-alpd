...
if ~exist('params')
    ...
    params.IsingleCh1=165;%157;%193;%165;%137;%200; %249;%225%234; %200 +- 20 mCherry in vivo M1 150319 (M1 Prime95b @ 50 mW, 5ms exposure, 7ms framerate, 12-bit Sensitivity mode)
    params.IsingleCh2=161;%114;%175;%200; +- 20 eGFP in vivo M1 150319 (M1 Prime95b @ 50 mW, 5ms exposure, 7ms framerate, 12-bit Sensitivity mode)
    params.SNRminG=0.4;
    params.SNRminR=0.4;
    params.overtrack=0;
    ...
    params.stoichKDW = 0.7;%molecules   %kernel density widths for plots
    params.diffKDW = 0.08;%um2/s
    
    % Half frame width (e.g. 512 x 512 => 256)
    params.framewidth = 600;%600;%64;
    
    % Adjusted offsets
    xoffset = 0; %(by images)[by max coloc]: Control Q3(+9/-8), Q5(+0,+7), Q7(+13,0); Pert Q1(+6,-7)[+9,-8], Q2(+6,-18)[+12,-14]; Tras Q1(+11,-18)[+14,-13], Q2(+14,-2)[+13,-4]
    yoffset = 0;
    
    %params.alignmentTransform=[xoffset-params.framewidth,-yoffset]; %for general offsets from ImageMakerAligned;
    
    %if 0, then summed cyto, =1 then summed nuc,=2 then cyto proper
    params.compartmentSwitch=2;
    %params.compartmentSwitch=2;
    ...
    params.uppertracklimit = 4;
    
    params.frameLimit=5; %number of frames to use (typically first 5 after laser on, to avoid photobleached stoichiometry)
    params.pixelSize=0.053;%53; %pixel size in microns (11um pixel / 100x objective / 2x telescope)
    params.frameTime=0.00744; %frame time in seconds  (e.g. 5 ms exposure + readout of 2.5ms)
    params.PSFsize=200; %size of PSF in nm
    % any co-ordinate transformation between the two channels
    params.transform=[0,0];
    params.colocError=4;%2.5; %4; %colocalisation error in pixels
    
    % maximum distance in pixels to link spots across channels
    params.d=8;
    % minumum overlap integral to link spots across channels
    params.overlap=0.9;%0.75
    %params.overlap=0.9;
            ...
            switch ch
                case 1
                    ...
                    if isempty(SpotsCh1) || size(SpotsCh1,2)<12
                        ...
                        SpotsCh1=SpotsCh1(SpotsCh1(:,11)>params.SNRminG,:); %screen low SNR spots
                        ...
                        if SpotsCh1(1,5)==0
                            ...
                            [trackArrayCh1,spotsInTracksCh1,segArray,cellNo1]=trackAnalyser5_ALH_general(spots,segObject,trackArrayCh1,params.frameLimit,params.frameTime,params.pixelSize,tracksFile,params.PSFsize,segArray,cellNo1,params.lowertracklimit,params.uppertracklimit);
                        end
                        emptyCh1=0;
                    end
                    end
                case 2
                    if ~isempty(whos(m1,'SpotsCh2')) && length(m1.SpotsCh2)>=12
                        ...
                        SpotsCh2=SpotsCh2(SpotsCh2(:,11)>params.SNRminR,:); %screen low SNR spots
                        if isempty(SpotsCh2)  || size(SpotsCh2,2)<12
                            ...
                            if SpotsCh2(1,5)==0
                                ...
                                %                             if ~exist('segObject') || isempty(segObject)
                                %
                                %                                 segObject=ones(ceil(max(spots(:,1))),ceil(max(spots(:,2))));
                                %
                                %                             end
                                %  segObjectCh2=segObject;
                                SpotsCh2temp =spots;
                                SpotsCh2temp(:,13)=l;
                                AllSpots2=cat(1,AllSpots2,SpotsCh2temp);
