...
%Use the output transform as 'p.beadtform' in the TrackAll7 code.
...
paramsbead.useavgproj = 1; %flag, if 0 then use first frame of bead video only, if 1 then use mean projection over all time frames.
...
if paramsbead.useavgproj==1
    bead_frame = mean(bead_image,3); % 3 refers to time dimension
else 
    bead_frame = bead_image(:,:,1); % first frame only
end

%% USE THIS LINE FOR <v2020a
[beadsframech1,beadsframech2,SlitSegmentObject]=prepRegImages(bead_frame,paramsbead);
%% USE THIS LINE FOR >v2021a
...
cropfilenameby = 21; %characters for '_MMStack_Pos0,ome.tif' standard on M1 microscope
