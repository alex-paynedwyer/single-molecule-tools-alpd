function [x_centre, y_centre, clipping_flag, Ibg_avg, Isp, Idata, bg_noise_std,  mask_pixels, noConvergenceFlag]= ...
    findSpotCentre4(image_data,x_estimate,y_estimate,p, clip_override)
% 
% New functionality if in cursor mode
%
% if SNR<threshold during cursor mode, will return initial values and guess
% for Isp
%
...
inner_radius=p.inner_circle_radius;
sigma_gauss=p.gauss_mask_sigma;
show_output=p.show_all_output;
error_set=p.error_set;
...
if p.use_cursor==1 && snr1<p.SNR_min
    if clip_override==1
        x_centre=x_estimate;
        y_centre=y_estimate;
        %     clipping_flag=0;
